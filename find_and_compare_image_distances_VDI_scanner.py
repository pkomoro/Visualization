...
if __name__ == "__main__":
    ...
    lens1 = Lens(focal_length_adjusted, optics_diameter_adjusted, object_positions)
    lens_power_efficiency = beam1.power_through_aperture(optics_diameter_adjusted / 2, object_positions) # Overlap of the lens aperture with the source beam
    ...
    image_positions_thin_lens = thin_lens_equation(object_positions, focal_length_adjusted)
    ...
    if d_values:
        ...
        ax_left.axvline(2 * focal_length_adjusted, color='gray', linestyle='--', linewidth=1)
        ax_left.axhline(2 * focal_length_adjusted, color='gray', linestyle='--', linewidth=1)
        # label the 2f lines on the bottom and left axes
        xlim = ax_left.get_xlim()
        ylim = ax_left.get_ylim()
        ax_left.text(2 * focal_length_adjusted, ylim[0], ' 2f', color='gray', ha='left', va='bottom')
        ax_left.text(xlim[0], 2 * focal_length_adjusted, ' 2f', color='gray', ha='left', va='bottom')
